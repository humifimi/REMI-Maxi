/**
 * Datetime contract helpers for the REMITechnician app.
 *
 * THE CONTRACT
 * ────────────
 * Backend `timestamptz` columns (e.g. `personal_events.start_time`,
 * `appointments.created_at`) ALWAYS receive a fully-qualified ISO 8601
 * string with an explicit timezone marker — either `Z` for UTC or a
 * `±HH:MM` offset. Producers (form submits, scheduler payloads) MUST
 * NOT build these strings with naive template interpolation like
 * `` `${date}T${time}:00` `` because:
 *
 ...
 * with values returned from the backend.
 *
 * SIBLING REPOS
 * ─────────────
 * This file is the source of truth for REMITechnician. The function
 * signatures are intentionally portable so REMICustomer, REMIDashboard,
 * and REMIBackend (for tests/seeders) can drop in identical copies.
 * If you change a signature here, update them in lockstep.
 *
 * See: `.cursor/rules/datetime-and-data-format-contracts.mdc` (the rule
 * that codifies this contract repo-wide and forbids the naive pattern).
 */
 ...
 * for ANY field declared as a backend `timestamptz` (e.g.
 * `personal_events.start_time`, `personal_events.end_time`).
 *
 * NOTE: do NOT use for `appointments.scheduled_time` — that's a
 * Postgres `TIME` (TZ-naive) column and arrives as `"06:00:00"`. Pass
 * those through `timeStringToMinutes` in `resource-calendar-mapping`
 * instead. The split is intentional: the column type determines the
 * helper, not the field name.
 *
 ...
 * originally produced (e.g. the personal-event edit sheet). DO NOT
 * `.slice(0, 5)` an ISO string and call it a time — that returns
 * "2026-" or similar garbage.
 *
 * @param iso A fully-qualified ISO 8601 string from the backend
