...
const ONBOARDING_STEP_ROUTES: Record<OnboardingStepId, string> = {
  ...
  addVehicle: '/customer/vehicle/add?from=onboarding',
  garageConfirmation: '/customer/garage-confirm',
  schedulePreferences: '/customer/schedule-prefs',
  notificationPreferences: '/customer/notification-prefs',
...
export function getOnboardingResumeRoute(
  ...
  const next = ONBOARDING_STEPS.find((step) => !completedSteps[step]);
  return next ? ONBOARDING_STEP_ROUTES[next] : '/customer';
}
